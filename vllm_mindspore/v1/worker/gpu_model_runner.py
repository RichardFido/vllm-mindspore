...
from vllm_mindspore.v1.attention.backends.ms_attn import MsCommonAttentionMetadata
...
def _prepare_inputs(
    ...
    scheduler_output,
           ...
           np.ndarray, Optional[CommonAttentionMetadata], int, Optional[UBatchSlices],
               Optional[Tensor]]:
    """
    :return: tuple[
    ...
    self.input_ids.np[:total_num_scheduled_tokens] = np.take(self.input_batch.token_ids_cpu.ravel(), token_indices, 0)
    # vllm-mindspore end
...
def _allocate_nz_kv_cache_tensors(self, kv_cache_config):
    ...
    for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
        ...
        coef = 1 if isinstance(
            kv_cache_spec, (MLAAttentionSpec, MLAQuantFullAttentionSpec)) else 2
        for _ in range(coef):
...
def _allocate_kv_cache_tensors(self, kv_cache_config):
    ...
    # Determine the number of tensors in kv_cache using the type of kv_cache_spec.
    coef = 1 if isinstance(
        kv_cache_spec, (MLAAttentionSpec, MLAQuantFullAttentionSpec)) else 2
...
def _reshape_kv_cache_tensors(
    ...
    for group in self._kv_cache_spec_attn_group_iterator():
        ...
        # Determine the number of tensors in kv_cacheusing the type of kv_cache_spec.
        coef = 1 if isinstance(
            kv_cache_spec, (MLAAttentionSpec, MLAQuantFullAttentionSpec)) else 2
        ...
        for layer_name in group.layer_names:
            ...
            if isinstance(kv_cache_spec, (FullAttentionSpec, MLAQuantFullAttentionSpec)):
                kv_cache_shape = attn_backend.get_kv_cache_shape(
                    num_blocks, kv_cache_spec.block_size,
                    kv_cache_spec.num_kv_heads, kv_cache_spec.head_size, cache_dtype_str=self.cache_config.cache_dtype)
                dtype = kv_cache_spec.dtype
                try:
